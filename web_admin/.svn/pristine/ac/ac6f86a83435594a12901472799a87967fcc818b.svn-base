<div class="bootbox modal fade in" id="myModal" tabindex="-1" role="dialog" open-close-modal
     aria-labelledby="myModalLabel" aria-hidden="true" ng-controller="optionalTpl" ng-cloak>
      ...
      <form name="myform" novalidate="novalidate">
        ...
        <div class="modal-body" id="aa">
            ...
            <table class="table table-striped table-bordered table-hover table-width">
              ...
              <tr>
                <td colspan="5" ng-show="!storeList.length" class="red text-center" ng-cloak>
                  暂无数据
                </td>
              </tr>
              </tbody>
        ...
        <div class="modal-footer">
          <a class="btn btn-default" data-dismiss="modal">取消</a> <a
            class="btn btn-primary" ng-click="btnConfirm()">确定</a>
        </div>
      </form>
    </div>
...
<script type="text/javascript">
  app.controller("optionalTpl", function ($scope, $timeout, $rootScope, $http) {
    var int = 1;
    $scope.page = {};
    var arrayStory = [];
    //选择场景
    function tplList(int) {
      $http.post(wsh.url + "get-buyer-optional-tpl-list", {
        "_page": int,
        "_page_size": 10
    ...
    $rootScope.btnConfirm = function () {
      var int = 0;
      for (var i in $scope.storeList) {
        if (!$scope.storeList[i].isshow && $scope.storeList[i].ischeck) {
          int = 1;
          continue;
        }
      }
      if (!arrayStory.length) return alert('请选择模版!!');
      $rootScope.$broadcast('chooseOptionTpl', arrayStory);
