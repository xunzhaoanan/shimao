...
namespace common\vendor\wechat\wechat_sdk;
...
class Wechat
{
    ...
    const EVENT_CARD_USER_VIEW = 'user_view_card';          //卡券 - 审核通过
    const EVENT_CARD_PASS = 'card_pass_check';          //卡券 - 审核通过
    const EVENT_CARD_NOTPASS = 'card_not_pass_check';   //卡券 - 审核未通过
    const EVENT_CARD_USER_GET = 'user_get_card';        //卡券 - 用户领取卡券
    const EVENT_CARD_USER_DEL = 'user_del_card';        //卡券 - 用户删除卡券
    const EVENT_CARD_CONSUME_CARD = 'user_consume_card';//卡券 - 用户核销卡券
    const EVENT_POI_CK_NOTIFY = 'poi_check_notify';     //门店审核
    const EVENT_WIFI_CONNECTED = 'WifiConnected';       //用户wifi连接成功
    ...
    const MEDIA_IMAGE_UPLOAD = '/media/uploadimg?';
    ...
    public function validation(){
        if ($this->checkSignature()){
            BaseCache::append('test_cache',time());
     ...
     * 获取接收短视频推送
     */
    public function getRevShortVideo(){
        if (isset($this->_receive['MediaId'])){
            return array(
                'mediaid'=>$this->_receive['MediaId'],
                'thumbmediaid'=>$this->_receive['ThumbMediaId']
	...
	public function getRevCardGet(){
        ...
        if (isset($this->_receive['FriendUserName']) && !empty($this->_receive['FriendUserName']))    //转赠用户
            $array['FriendUserName'] = $this->_receive['FriendUserName'];
        ...
        if (isset($this->_receive['OldUserCardCode']) && !empty($this->_receive['OldUserCardCode']))    //转赠用户卡券code
     ...
     * @return array|boolean
     */
    public function getUserCardView(){
        if (isset($this->_receive['ToUserName']))
            $array['ToUserName'] = $this->_receive['ToUserName'];
        if (isset($this->_receive['FromUserName']))
    ...
    public function reply($msg=array(),$return = false)
    {
        ...
        if ($return) {
            ...
            BaseCache::append('test_cache',__FILE__.'_____'.__LINE__);
            BaseCache::append('test_cache',time(),true);
            BaseCache::append('test_cache',$xmldata,3600,true);
            Yii::info($xmldata, 'wxapi');
            exit($xmldata);
        }
    }
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param string $url
     */
    private function http_get($url, $refresh = true){
        $oCurl = curl_init();
        ...
        if(intval($aStatus["http_code"])==200){
            ...
            if(isset($json['errcode']) && ($json['errcode'] == 40001 || $json['errcode'] == 42001 || $json['errcode'] == 40014) && $refresh){
                $url = preg_replace_callback("/access_token=([^&]*)/i", function () {
                    return 'access_token=' . WechatCache::getToken($this->appid,$this->appsecret,true);
                }, $url);
     ...
     */
    private function http_post($url,$param,$post_file=false,$refresh = true){
        $oCurl = curl_init();
        if(stripos($url,"https://")!==FALSE){
            curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
            curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
        ...
        $sContent = curl_exec($oCurl);
        BaseCache::append('test_cache',$url,600,true);
        BaseCache::append('test_cache',$param,600,true);
        BaseCache::append('test_cache',$sContent,600,true);
        $aStatus = curl_getinfo($oCurl);
        curl_close($oCurl);
        if(intval($aStatus["http_code"])==200){
            Yii::error($sContent, 'wechat');
            $json = json_decode($sContent,true);
            if(isset($json['errcode']) && ($json['errcode'] == 40001 || $json['errcode'] == 42001 || $json['errcode'] == 40014) && $refresh){
                $url = preg_replace_callback("/access_token=([^&]*)/i", function () {
                    return 'access_token=' . WechatCache::getToken($this->appid,$this->appsecret,true);
                }, $url);
                return $this->http_post($url,$param,$post_file,false);
            }else {
                return $sContent;
            }
        }else{
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $this->access_token = WechatCache::getToken($appid,$appsecret);
        return WechatCache::getToken($appid,$appsecret);
    }
     ...
     */
	public function resetJsTicket($appid='', $type='jsapi'){
        
        if (!$appid) $appid = $this->appid;
        $this->jsapi_ticket = '';
		$authname = $type.'_ticket_'.$appid;
     ...
     * @param string $type 默认jsapi,调用微信卡券接口时用wx_card
	...
	public function getJsTicket($appid='', $jsapi_ticket='', $type = 'jsapi'){
        ...
        if ($result)
        {
            ...
            BaseCache::set($authname,$this->jsapi_ticket,$expire);
            return $this->jsapi_ticket;
        }
        return false;
    }
    ...
    public function getJsSign($url, $timestamp=0, $noncestr='', $appid=''){
        ...
        BaseCache::append('test_cache',__FILE__.'_____'.__LINE__);
        BaseCache::append('test_cache', $arrdata);
        if (!$sign)
            return false;
        $signPackage = array(
            "appid"     => $this->appid,
    ...
    public function uploadMedia($data, $type){
        ...
        $data = new \CURLFile($data);
        $mediaData = array("media"  => $data);
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$mediaData,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
...
//        BaseCache::append('test_cache',$url_prefix.self::MEDIA_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id);
//        BaseCache::append('test_cache',__FILE__.'_____'.__LINE__);
//        BaseCache::append('test_cache',$result);
        if ($result)
        {
            if (is_string($result)) {
                $json = json_decode($result,true);
    ...
    public function uploadForeverMedia($data, $type,$is_video=false,$video_info=array()){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$mediaData,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
...
//        BaseCache::append('test_cache',__FILE__.'_____'.__LINE__);
//        BaseCache::append('test_cache',self::json_encode($data));
    ...
    public function uploadMpImage($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::UPLOAD_MEDIA_URL.self::MEDIA_IMAGE_UPLOAD.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function getUserInfo($openid){
        BaseCache::append('test_cache','$this->access_token'.$this->access_token);
        if (!$this->access_token && !$this->checkAuth()) return false;
        BaseCache::append('test_cache','$this->access_token'.$this->access_token);
