  ...
  <div class="main-container-inner"> <?php echo $this->render('@app/views/side/weixin_setting.php');?>
    <div class="main-content">
      ...
      <div class="page-content"> 
                      ...
                      <div class="inner"> 
                        ...
                        <div class="form-group margin-bottom10 clearfix">
                          ...
                          <div class="frm_input_box with_counter counter_in append count">
                            <input maxlength="30" type="text" class="form-control" placeholder="请输入标题" required="required" name="title" ng-model="model.title">
                        ...
                        <div class="form-group margin-bottom10 clearfix">
                          ...
                          <div class="frm_textarea_box with_counter counter_out counter_in append count com_form">
                            <div class="face-box">
                               <span class="block text-hint text-right">{{'还可以输入' + (model.reply_content.length ? (300 - model.reply_content.length) : 300) + '字'}}</span>
                              </div>
                              <textarea class="input" name="reply_content" ng-model="model.reply_content" required="required" maxlength="300"></textarea>
                              <span class="block red" ng-show="myform.reply_content.$error.required && istrue">必填项</span>
                              <span class="block red" ng-show="myform.reply_content.$error.maxlength">字符过多</span> 
                          </div>
        ...
        <div class="modal-footer margin-auto" id="modal-footer"> <a ng-click="save()" class="btn btn-primary" id="submit">保存并关闭 </a> </div>
      </div>
    </div>
    <!-- /.col --> 
  </div>
...
app.controller('mainController', function($scope, $http, $rootScope, $timeout){
  $timeout(function(){$rootScope.$broadcast('leftMenuChange','ba');},100);
	...
	$scope.save = function(){
        ...
        $http.post("/wxmaterial/text-edit-ajax", $scope.model)
