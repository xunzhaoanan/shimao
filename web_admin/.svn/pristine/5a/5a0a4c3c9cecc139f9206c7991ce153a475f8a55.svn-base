...
class BaseModelBak extends ActiveRecord
{
    ...
    public function getResult($urlKey, $params , $filter = true)
    {
        $this->_data = null;
        $this->errorMsg = null;
        $url = Yii::$app->params['baseApiRootUrl'] . Yii::$app->params['apiUrls'][$urlKey];
        $apiData = $this->postCurl($url, $params ,$filter);
    ...
    private function _ajaxHttp($url, $param)
    {
        ...
        curl_setopt($oCurl, CURLOPT_URL, $url);
        curl_setopt($oCurl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($oCurl, CURLOPT_TIMEOUT_MS, 18);//超时为18毫秒
        curl_setopt($oCurl, CURLOPT_POST, true);
        curl_setopt($oCurl, CURLOPT_POSTFIELDS, $strPOST);
        curl_exec($oCurl);
        curl_close($oCurl);
