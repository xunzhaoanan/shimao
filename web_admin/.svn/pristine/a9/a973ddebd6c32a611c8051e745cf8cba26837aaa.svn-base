...
<div class="sidebar sidebar-fixed" id="sidebar"> 
  ...
  app.controller('leftController', function($scope, $rootScope){
      $scope.leftArray = $rootScope.headList.h.child;
      $scope.setHref = function(childList){
        if(childList.child){
          var href = ''
          for(var i in childList.child){
            if($rootScope.permissionJSON.indexOf(childList.child[i].keyH) != -1){
              href = childList.child[i].href;
              break;
            }
          }
          return href;
        }else{
          return childList.href
        }
      }
    ...
    $rootScope.$on('leftMenuChange', function(e, id){
      ...
      for(var i = 0;  i< $('#headactive li').length; i++ ){
        if($('#headactive li').eq(i).find('a').text() == '微分店'){
          index = i;
        }
      }
