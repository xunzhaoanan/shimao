...
class Zip {
    ...
    public function __construct(){
        $baseDir = \Yii::$app->basePath.DIRECTORY_SEPARATOR.'web'.DIRECTORY_SEPARATOR.'zip'.DIRECTORY_SEPARATOR;
        if( ! file_exists($baseDir)){
            mkdir($baseDir) or die('没有权限');
        }
        $this->baseDir = $baseDir;
    }

    public function down($get_url,$fileName = '扫码支付二维码.zip'){
