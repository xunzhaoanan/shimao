...
<div class="sidebar sidebar-fixed" id="sidebar"> 
  ...
  <!-- #sidebar-shortcuts -->
    <ul class="nav nav-list" id="navList" ng-controller="leftController">
        <li ng-class="{true: 'open'}[$root.leftMenuIndex == $index]" ng-repeat="list in leftArray" ng-if="$root.authorityJSON[list.key] == 1" pid="{{list.key}}"> <a ng-click="clickMenu($index, $event, list)" ng-href="{{list.href}}" ng-class="{true: 'dropdown-toggle'}[list.child.length > 0]"> <i class="{{list.class}}"></i> <span class="menu-text" ng-bind="list.text"></span><b class="arrow icon-angle-down" ng-if="list.child.length"></b> </a>
            <ul ng-class="{true: 'submenu', false: 'submenu hide'}[list.isshow]" ng-if="list.child.length" style="display:block;">
                <li ng-repeat="childList in list.child track by $index" ng-if="$root.authorityJSON[list.key] == 1"> <a href="{{childList.href}}" ng-bind="childList.text"> <i class="icon-double-angle-right"></i></a> </li>
            </ul>
        </li>
    </ul>
...
?>
            ...
            $scope.leftArray = [
                 {href: '', text: '直营店管理', id: 'terminal_list', class: 'icon-dailishang newicon-font', isshow: false, key: 'ha'},
                 {href: '', text: '直营店数据统计', id:'data_order_shop', class: 'icon-dailishang newicon-font', isshow: false, key: 'hb', child: [
                    {href: '/data/order-shop', text: '订单统计', class: 'icon-line-chart', key: 'hba', child: []},
                    {href: '/data/member-shop', text: '客户统计', class: 'icon-pie-chart', key: 'hbb', child: []},
                    {href: '/data/fx-shop', text: '推广统计', class: 'icon-pie-chart', key: 'hbc', child: []}
                 ]},   
                 {href: '', text: '代理商管理', id:'agent_list', class: 'icon-dailishang newicon-font', isshow: false, key: 'hc'},
                 {href: '', text: '代理商数据统计', id:'data_agent_order_agent', class: 'icon-shuju newicon-font', isshow: false, key: 'hd', child: [
                    {href: '/data/agent-order-agent', text: '订单统计', class: 'icon-shopping-cart', key: 'hda'},
                    {href: '/data/agent-order-detail', text: '订单明细统计', class: 'icon-shopping-cart', key: 'hdb'},
                    {href: '/data/agent-member-agent', text: '客户统计', class: 'icon-shopping-cart', key: 'hdc'},
                    {href: '/data/agent-member-detail', text: '客户明细统计', class: 'icon-shopping-cart', key: 'hdd'},
                    {href: '/data/agent-fx-agent', text: '推广统计', class: 'icon-shopping-cart', key: 'hde'},
