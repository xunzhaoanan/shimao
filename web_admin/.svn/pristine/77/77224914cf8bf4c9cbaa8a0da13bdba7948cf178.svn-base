...
<div class="sidebar sidebar-fixed" id="sidebar"> 
  ...
  app.controller('leftController', function($scope, $rootScope){
    $scope.leftArray = [
             {href: '/magazine/list', text: '杂志管理', class: 'icon-yingyongtuiguang newicon-font', isshow: false, key: 'fa'},
             {href: '/magazine/category-list', text: '分类管理', class: 'icon-mingxi newicon-font', isshow: false, key: 'fb'},
             {href: '/magazine/form', text: '表单数据', class: 'icon-shuju newicon-font', isshow: false, key: 'fc'}
    ];
    $scope.leftArray = $rootScope.headList.f.child;
    ...
    $rootScope.$on('leftMenuChange', function(e, id){
      ...
      for(var i = 0;  i< $('#headactive li').length; i++ ){
        if($('#headactive li').eq(i).find('a').text() == '微杂志'){
          index = i;
        }
      }
