              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" class="weiright-td ">
                <tr>
                  <td colspan="2"  valign="top" class="no-padding">
                    <h4 class="header smaller lighter blue no-margin-bottom"> <span class="badge badge-danger"></span> 设置关键词</h4>
                  </td>
                </tr>
                <tr>
                  <td align="right" class="width101 " valign="middle"><span class="red">*</span> 关键字</td>
                  <td><input type="text" class="col-sm-4" name="name" ng-model="model.keyword" required="required" ng-maxlength="50">
                    <span class="inline red padding5" ng-show="myform.name.$error.required && istrue">必填项</span>
                    <span class="inline red padding5" ng-show="myform.name.$error.maxlength">字符过多</span>
                ...
                <tr>
                 ...
                 <tr>
                  ...
                  <td><select ng-model="model.match_type" ng-options="o.id as o.name for o in matchSelect">
                    </select></td>
                </tr>
              </table>
            </div>
            <div class="col-sm-6">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" class="weiright-td tjhf">
                <tr class="no-border">
                  <td colspan="2" valign="top" class="no-border no-padding">
                    <h4 class="header smaller lighter blue no-margin-bottom"> <span class="badge badge-danger"></span> 添加回复消息</h4>
                  </td>
                </tr>
                  ...
                  <td valign="top" ng-class="{true: 'zdhf'}[list.desc == '语音']">
                    ...
                    <div class="inline align-top width90">
                      <div class="margin-bottom5 text-overflow">
                         <span class="label label-sm label-primary weibq margin-right5" ng-bind="list.desc"></span>
                         <strong>{{list.title}} </strong>
                      </div>
                      <a class="table-width" ng-bind-html="list.reply_content | sysface | trust:'html'" ng-if="list.desc == '文本'"></a>
                      <div class="dtw_tu" ng-if="list.desc == '语音'"></div>
                      <span ng-if="list.desc == '语音'">4"</span>
                      <img ng-src="{{list.cdn_path}}" class="align-top" width="45" height="45" ng-if="list.desc == '图片'">
                      <img ng-src="{{list.wxImagetxtReplyItems[0].cdn_path}}" class="align-top" width="45" height="45" ng-if="list.desc == '图文'"> 
                    </div>
                  </td>
                  <td class="visible-md visible-lg hidden-sm hidden-xs action-buttons width-60px">
                    <a class="blue pointer" ng-click="history($index)" data-toggle="modal" data-target="#insertMaterial" title="编辑"> <i class="icon-编辑 bigger-130"></i></a> 
                    <a class="red pointer" title="删除" ng-click="Delete($index)"> <i class="icon-shanchu bigger-130"></i></a></td>
                </tr>
                <tr>
                  <td  valign="top" colspan="2" class="no-border text-right">
                    <a class="btn btn-primary" data-toggle="modal" data-target="#insertMaterial" ng-click="restHistory()" ng-show="MaterialLists.length < 5"> 添加一条消息 </a></td>
                </tr>
              </table>
            </div>
          </div>
          </form>
          <div class="modal-footer margin-auto" id="modal-footer"> <a class="btn btn-primary" ng-click="save()"> 确定 </a></div>
        </div>
        <!-- /.col --> 
      </div>
      <!-- /.col --> 
...
<!-- 插入素材 -->
<div class="bootbox modal fade in"  id="insertMaterial" tabindex="-1" role="dialog" open-close-modal aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog6">
    <div class="modal-content">
      ...
      <a data-bb-handler="confirm" class="btn btn-primary" ng-click="$root.getWxmaterials()">确定</a> </div>
    </div>
  </div>
</div>
<script>
app.controller('mainController', function($scope, $http, $rootScope, $timeout){
	$timeout(function(){$rootScope.$broadcast('leftMenuChange', 'bb');}, 100);
	$scope.istrue = false;
	...
	$('#select').find('option').each(function(i, e) {
        if(e.value == $scope.model.match_type) $(e).attr('selected', 'selected');
    });
...
//		$rootScope.Wxmaterials.wxImagetxtReplyItems[0] = $rootScope.Wxmaterials.wxImagetxtItem;
    ...
    $scope.save = function(){
		...
		$.ajax({
		   type: "POST",
		   url: "/weixin/keyword-reply-edit-ajax",
		   data: $scope.model,
		   dataType:"json",
		   success: function(msg){
			   $('#submit').removeAttr('disabled');
			   wsh.successback(msg, '提交成功', false, function(){window.location.href = 'keyword-reply-list';});
		   },
		   error: function(){
			   $('#submit').removeAttr('disabled');
			   alert('服务器忙');    
