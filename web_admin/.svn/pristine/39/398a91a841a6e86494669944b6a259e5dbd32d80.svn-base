            ...
            <div class="clearfix no-padding">
              <div class="col-sm-7 no-padding">
                <ul class="listli left-space1 btn-primary bune">
                  <li><a href="/market-activity/add"
                         ng-if="$root.hasPermission('market-activity/add')"
                         class="btn btn-xs btn-primary">新增活动</a></li>
                </ul>
              </div>
            </div>
                ...
                <tr ng-repeat="list in lists" ng-cloak>
                  ...
                  <td class="text-center"><label>
                    <input name="switch-field-1"
                           ng-disabled="!$root.hasPermission('market-activity/open-ajax')"
                           class="ace ace-switch ace-switch-6" ng-model="list.isDelete"
                           type="checkbox" ng-click="btnStatus(list)">
                    <span class="lbl"></span> </label></td>
                  <td class="text-center">
                    <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                      <a target="_blank"
                         ng-href="{{'/activity/qrcode?model=marketing_activity&model_id='+list.id}}"
                         class="blue pointer "><i
                          class="icon-erweima bigger-130 text-decoration"></i></a>
                      <a class="grey pointer " href="/market-activity/winner-record?id={{list.id}}"
                         title="中奖名单"
                         ng-if="$root.hasPermission('market-activity/winner-record')"><i
                          class="icon-renyuanjieshao bigger-130 text-decoration"></i></a>
                      <a ng-href="{{'/market-activity/edit?id=' + list.id}}" class="pointer"
                         ng-if="$root.hasPermission('market-activity/edit')" title="编辑"><i
                          class="icon-bianji bigger-130 text-decoration"></i></a>
                      <a class="red pointer" title="删除" ng-click="btnDelete(list)"
                         ng-if="$root.hasPermission('market-activity/del-ajax')"> <i
...
<script>
  app.controller('mainController', function ($scope, $rootScope, $timeout) {
    ...
    function getData(int) {
      $.ajax({
        type: "POST",
        url: "<?= Url::to(['market-activity/list-ajax']);?>",
        dataType: "JSON",
        data: {
          "_page": int,
          "_page_size": 15
    ...
    $scope.btnStatus = function (list) {
      if (list.isDelete) {
        $.ajax({
          type: "POST",
          url: "<?= Url::to(['market-activity/open-ajax']);?>",
          dataType: "JSON",
          data: {
            "id": list.id
          },
        ...
        $.ajax({
          type: "POST",
          url: "<?= Url::to(['market-activity/close-ajax']);?>",
          dataType: "JSON",
          data: {
            "id": list.id
          },
    ...
    $scope.btnDelete = function (list) {
      wsh.setDialog('活动删除提示', '确定要删除此活动吗?', '/market-activity/del-ajax', {"id": list.id}, function () {
        getData(1)
      }, '删除成功');
    }
