          ...
          <div class="col-xs-12"> 
            ...
            <div class="tabbable">
              <ul class="nav nav-tabs" id="myTab">
                ...
                <li class="active" style="cursor:pointer;"><a href="/document/music">音乐文件</a> </li>
              ...
              <div class="tab-content col-sm-12 clearfix">
                <div class="tab-pane active"> 
                  ...
                  <div class="form-group clearfix">
                    <div class="clearfix">
                      <ul class="listli left-space1 float-left col-sm-6 across-space1 no-margin-bottom margin-top3">
                        <li> <a data-toggle="modal" data-target="#myModalMusic" class="btn btn-xs btn-primary">上传音乐</a> </li>
                      </ul>
                      <div class="input-group float-right col-sm-2 across-space2">
                        ...
                        <span class="input-group-btn"><a href="#" class="btn btn-xs btn-primary" ><i class="icon-search icon-on-right bigger-110"></i></a></span> </div>
                    </div>
                  </div>
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover table-width">
                      ...
                      <tbody>
                        <tr ng-repeat="list in lists">
                          ...
                          <td ng-bind="list.name" class="musicEdit"> 382_m382_m382_m </td>
                          ...
                          <a data-toggle="modal" data-target="#myModal3" class="success" title="编辑" style="margin:0 6px;" ng-click="imageEdit($index)"> <i class="icon-pencil bigger-130"></i></a>
                          <a class="red" title="保存" style="margin:0 6px;" ng-show="list.isclick" ng-click="imageSave($index)"><i class="icon-save bigger-130"></i></a>
                          <a class="red" title="取消" style="margin:0 6px;" ng-show="list.isclick" ng-click="imageCancel($index)"><i class="icon-remove bigger-130"></i></a>
                          <a class="red" title="删除" style="margin:0 6px;"><i class="icon-trash bigger-130"></i></a> 
                          </div></td>
                        </tr>
                      </tbody>
                    </table>
...
<?php echo $this->render('@app/views/uploadImg/music.php'); ?> 
<script>
...
app.controller('mainController', function($scope, $timeout, $rootScope){
	...
	function getData(int, size){
		$.post('<?= Url::to(['/document/music-ajax']);?>', {'_page': int, '_page_size': size}, function(msg){
			if(msg.errcode == 0){
	...
	$scope.imageEdit = function(index){
		...
		$('.musicEdit').eq(index).attr('contenteditable', 'true').focus();
	};
	$scope.imageSave = function(index){
		$.post('<?= Url::to(['/document/edit-ajax']);?>', $scope.lists[index], function(msg){
			if(msg.errcode == 0){
				globalClick = false;
				$('.musicEdit').eq(index).attr('contenteditable', 'false').blur();
