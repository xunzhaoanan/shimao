...
<div class="sidebar sidebar-fixed" id="sidebar">
  ...
  <ul class="nav nav-list" id="navList" ng-controller="leftController">
    <li ng-class="{true: 'open'}[$root.leftMenuIndex == $index]" ng-if="list.hasPermission" pid="{{list.key}}" ng-repeat="list in leftArray">
    <a ng-href="{{list.href}}" ng-class="{true: 'dropdown-toggle'}[list.child.length > 0]" ng-click="clickMenu($index, $event, list)">
    <i class="{{list.class}}"></i>
    <span class="menu-text" ng-bind="list.text"></span>
    <b class="arrow icon-angle-down" ng-if="list.child.length"></b> </a>
      <ul ng-class="{true: 'submenu', false: 'submenu hide'}[list.isshow]" ng-if="list.child.length" style="display:block;">
        <li ng-repeat="childList in list.child track by $index" ng-if="childList.hasPermission">
        <a ng-href="{{childList.href}}" ng-bind="childList.text"> <i class="icon-double-angle-right"></i></a> </li>
      </ul>
    </li>
  </ul>
  ...
  app.controller('leftController', function($scope, $rootScope){
    $scope.leftArray = $rootScope.headList.e.child;
    ...
    $rootScope.$on('leftMenuChange', function(e, id){
      ...
      for(var i = 0;  i< $('#headactive li').length; i++ ){
        if($('#headactive li').eq(i).find('a').text() == '微营销'){
          index = i;
        }
      }
