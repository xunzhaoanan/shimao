...
<div class="sidebar sidebar-fixed" id="sidebar"> 
  ...
  <!-- #sidebar-shortcuts -->
	<ul class="nav nav-list" id="navList" ng-controller="leftController">
		<li ng-class="{true: 'open'}[$root.leftMenuIndex == $index]" ng-repeat="list in leftArray"> <a ng-click="clickMenu($index, $event, list)" ng-href="{{list.href}}" ng-class="{true: 'dropdown-toggle'}[list.child.length > 0]"> <i class="{{list.class}}"></i> <span class="menu-text" ng-bind="list.text"></span><b class="arrow icon-angle-down" ng-if="list.child.length"></b> </a>
...
?>
           ...
           $scope.leftArray = [
               <?php
           if(isset(Session::get(Session::SESSION_KEY_MENU)['member_manage'])) echo "{href: '/member/list', text: '客户列表', class: 'icon-huiyuan newicon-font', isshow: false, child: []},";
               ?>
            ];
        }
		...
		$rootScope.$on('leftMenuChange', function(e, index){
			$rootScope.leftMenuIndex = index;
			if(typeof $scope.leftArray[index] != 'undefined' && $scope.leftArray[index].child.length){
