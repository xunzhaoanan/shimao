      ...
      <div class="page-content">
        <div class="row">
          <div class="col-xs-12">
            ...
            <div class="tabbable" ng-if="isShow">
              <ul class="nav nav-tabs" id="myTab">
                <li><a href="/terminal/scan-pay">直营店</a></li>
                <li class="active"><a href="/terminal/scan-pay-agent">加盟店</a></li>
              </ul>
            </div>
              ...
              <div class="row">
                <div class="col-xs-12">
                  <div class="float-left margin-left10 no-padding">
                    <div class="col-sm-7 no-padding">
                      <ul class="listli left-space1 btn-primary bune">
                        <li class="dropdown">
                          <a data-toggle="dropdown"  ng-if="$root.hasPermission('terminal/bulid-qrcode-ajax')"
                  ...
                  <!--<div class="inline float-right margin-left10">-->
                    <!--<select class="width120"  ng-options="o.id as o.agent_name for o in agentList" ng-model="agent_id" ng-change="agentChange(agent_id)">-->
                    <!--</select>-->
                  <!--</div>-->
                </div>
              ...
              <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover table-width">
                  ...
                  <tr>
                    ...
                    <th width="6%" class="text-center">所属代理商</th>
                  ...
                  <tr ng-repeat="list in lists">
                    <td class="lt-width3 text-center" ng-click="checkFun(1, list, $event)"><label>
                      <input type="checkbox" class="ace" ng-model="list.isCheck" ng-click="checkFun(2, list, $event)">
                      <span class="lbl"></span> </label></td>
                    <td ng-bind="list.shopInfo.name" class="text-center"></td>
                    <td class="text-center">加盟店</td>
                    <td ng-bind="list.shopInfo.address" class="text-center"></td>
                    <td class="text-center" ng-bind="list.shopInfo.phone"></td>
		                <td class="text-center" ng-bind="list.shopAgents.agent_name"></td>
                    <td class="text-center" ng-bind="list.shopSubSetting.scan_count"></td>
                    <td class="text-center"><label>
                      <input name="switch-field-1" ng-disabled="!$root.hasPermission('terminal/scan-pay-open-ajax') ||  !$root.hasPermission('terminal/scan-pay-close-ajax')"
                             class="ace ace-switch ace-switch-6" ng-model="list.shopSubSetting.is_scan_pay" my-check-box
                             type="checkbox" ng-click="statueFun(list.shopSubSetting.is_scan_pay, list.shopInfo.shop_sub_id)">
                      <span class="lbl"></span> </label></td>
                    <td class="text-center">
                      <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons"><a
                          ...
                          ng-model="list.id"
                          ng-click="getQrcode(list.shopInfo.shop_sub_id)" data-toggle="modal" data-target="#query">
                        <i
                  ...
                  <tr>
                    <td colspan="9" ng-show="!lists.length" class="red text-center">暂时没有可展示的数据
                    </td>
                  </tr>
                  </tbody>
                </table>
...
<script type="text/javascript">
  app.controller('mainController', function ($scope, $rootScope, $timeout, $http) {
    $timeout(function () {
      ...
      $scope.searchName = '';
    var hrefid = wsh.getHref('agent_id'), agent_id = null;
    $scope.isShow = true;
    if(hrefid) $scope.isShow = false;
...
//    if(hrefid) agentselect = hrefid;
//    else agentselect = null;
//    $http.post('/agent/list-ajax', {'agent_id': agentselect})
...
//        }
//      }
//      $http.post('/terminal/list-ajax',{"_page": int, "_page_size": 20, 'shop_type':2, 'name': $scope.searchName, 'agent_id': agent_id})
      $http.post('/terminal/list-ajax',{"_page": int, "_page_size": 20, 'shop_type':2, 'name': $scope.searchName,'search_all':true})
          .success(function (msg) {
            wsh.successback(msg, '', false, function () {
              console.log(msg);
    ...
    $scope.checkFun = function (id, list, event) {
      if(id == 1){
        ...
        //console.log("111ggg", list.isCheck);
      }else if(id == 2){
        if (list.isCheck) {
          arrayStory.push(list.shopInfo.shop_sub_id);
        } else {
    ...
    $scope.handClick = function (list) {
      if(list.id == 1){  //所选
        ...
        if($scope.agent_id){
          ...
          location.href = '/terminal/bulid-qrcode-ajax?shop_type=2&name='+$scope.searchName+'&agent_id='+agent_id+'&search_all=true';
      }
    }
    ...
    $scope.normalSearch = function () {
      arrayStory = [];
      getData(1, $scope.searchName, $scope.agent_id);
    };
