          ...
          <div class="col-xs-12">
            ...
            <form class="form-horizontal" name="myform" novalidate="novalidate">
              <div class="form-group margin-bottom10 clearfix">
                ...
                <div class="col-xs-9">
                  <input placeholder="输入字符数不大于50" class="col-sm-3" type="text" ng-model="title"
                         name="title" required ng-maxlength="50">
                                    <span class="inline padding5 red"
                                          ng-show="myform.title.$error.required && isSubmit"
                                          ng-cloak>必填项</span>
                  <span class="inline padding5 red" ng-show="myform.title.$error.maxlength"
                        ng-cloak>字符个数不大于50</span>
                </div>
              </div>
...
//            window.location = 'list' + $rootScope.getSearchUrl;
            editRole(msg.errmsg[0].id);
          });
        },
        error: function (msg) {
          $('#btnSave').removeAttr('disabled');
