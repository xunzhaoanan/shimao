...
class CardCouponsController extends BaseController
{
    ...
    public function actionEdit()
    {
        ...
        if(trim($model['range'])){
           $model['range'] = array_map('intval', explode(',', trim($model['range'], ',')));
        }else{
            $model['range'] = [];
        }
    ...
    public function actionCardRecordListAjax()
    {
        ...
        // 客户资产明细
        if (isset($modelParams['member_detail'])){
            $modelParams['status'] = [CardCoupon::STATUS_CARD_RECEIVE, CardCoupon::STATUS_CARD_EXCHANGE]; //已领取或已核销的
            $modelParams['deleted'] = CardCoupon::STATUS_ENABLE;
            $modelParams['sortStr'] = ['modified' => 'desc', 'id' => 'asc']; //排序时，排序组合里出现相同比较值则会默认按id排序，这样就出现重复记录，故要加上按id排序
        }
        $this->cardCouponModel->findCardInfo($modelParams);
        // 接收逻辑层处理结果
        if (!is_null($this->cardCouponModel->getError())) {
